...
function sharedCaseDirectives(opts: { apiMode: boolean; apiContractRelevant: boolean; providerBehavior?: ProviderBehavior }): string[] {
  ...
  return [
    ...
    'Every case must include a standalone goal, inputs or test data when applicable, and expectedResult. expectedResult must be a concrete, FALSIFIABLE observable outcome that agrees with the BDD Then/And steps — it must fail against a WRONG implementation, not only a missing one. When the criterion or fixture involves a calculation, formula, specific value, format, count, ordering, or boundary, compute the exact expected output from the supplied inputs and assert THAT concrete result (the resulting number, string, row, or ordering), never a paraphrase of the rule or formula. If you assert a computed number, show the arithmetic in the Then/And steps and give the final value, e.g. "cell A: 0.6 × (250/1000) = 0.15; cell B: 0.4 × (800/800) = 0.40; total weight = 0.55".',
    'When a criterion involves a division or ratio, a denominator/divisor, a required or non-null numeric field, ordering/tie-breaking, or a boundary limit, add at least one edge or negative case (caseIntent "edge" or "negative") that exercises the degenerate input — a zero or null denominator/divisor, an empty input set, equal values that force a tie-break, or the boundary value itself — and assert the defined handling (skip, fallback, or error), explicitly confirming no divide-by-zero, NaN, or unhandled crash.',
    'For a criterion that requires existing behavior to "remain unchanged" (a regression guard), the case must establish a concrete baseline — specific pre-change values, a captured snapshot, or a named expected row/column set — and assert the post-change output EQUALS that baseline. Do not write a case whose expectedResult is merely "unchanged" or "same as before" with nothing concrete to compare against.',
