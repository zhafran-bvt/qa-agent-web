...
const { findCaseArray, normalizeBddScenario, normalizeCase, normalizeTextList, isFallbackError } = require('./llm');
...
test('normalizes structured BDD objects into Gherkin text', () => {
  assert.equal(
    normalizeBddScenario({
      ...
      Given: [
        'the user opens the line dataset selection screen',
        'the Adm Area filter is required for the dataset',
      ],
